...
if current_player.hand:
    render_cards(current_player.hand, height=170)
    
    st.markdown("**Select card to play:**")
    cols_per_row = 5
    for row_start in range(0, len(current_player.hand), cols_per_row):
        cols = st.columns(cols_per_row)
        for i in range(cols_per_row):
            idx = row_start + i
            if idx < len(current_player.hand):
                card = current_player.hand[idx]
                with cols[i]:
                    if card.kind == "money":
                        st.caption(f"💵 ${card.value}M")
                    elif card.kind == "property":
                        st.caption(f"🏠 {(card.active_color or 'Wild')[:6]}")
                    elif card.kind == "action":
